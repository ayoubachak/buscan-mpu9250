...
void can_send_temperature(void);
void can_send_accel(void);
void can_send_gyro(void);
void can_send_accel_g(void);
void can_send_gyro_rad_s(void);
void transform_accel(void);
void transform_gyro(void);
...
int main(void)
{
   ...
   // TEST MOTEUR
    ...
    dxl_setGoalVelocitcan_send_temperature()y(1, 140);
    HAL_Delay(5000);
...
void can_send_accel_g(){
	...
	for(int i = 0; i< 3; i++){
		accel_data_g[i] = (int16_t)( (float)accel_data[i]*(4.0 / 32768.0)*10000.0);
	}
...
void VL6180x_Init(void)
{
    ...
    XNUCLEO6180XA1_Init();
    HAL_Delay(500); // Wait
    // RESET	 term_printf("Acceleration Lineaire: %d", (int)accel_data);
	 term_printf("Acceleration Angulaire: %d", (int)gyro_data);
	 term_printf("Temperature : %d", (int)temp_data);
...
void VL6180x_Step(void)
{
    ...
    if (new_switch_state != switch_state) {	 term_printf("Acceleration Lineaire: %d", (int)accel_data);
	 term_printf("Acceleration Angulaire: %d", (int)gyro_data);
	 term_printf("Temperature : %d", (int)temp_data);
